"""
对比验证 Qwen-2-7b-Instruct 是否支持Anthropic Messages API

测试流程（严格对比）:
1. 直接向 OpenRouter 发送 OpenAI格式 → 确认模型可用
2. 直接向 OpenRouter 发送 Anthropic格式 → 确认是否原生支持
3. 通过 LiteLLM 发送 Anthropic格式 → 验证 LiteLLM 转换能力
"""
...
if openai_response.status_code == 200:
    ...
    print(f"❌ OpenRouter 不支持或模型不可用：{openai_response.status_code}")
    print(f"   错误：{openai_response.text[:200]}")
    model_available = False

print()

# ========== 步骤 2: 直接测试 Anthropic格式 ==========
...
print(f"   模型：qwen-free (LiteLLM 配置别名)")
